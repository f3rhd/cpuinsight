...
public:
	...
	void					d_cache_commit(store_instruction_t::STORE_INSTRUCTION_TYPE mode, memory_addr_t mem_addr, data_t data);
	void			  		reg_file_commit(const reg_id_t& reg_id, data_t data);
	data_t	  			d_cache_read(load_instruction_t::LOAD_INSTRUCTION_TYPE mode, memory_addr_t addr);
